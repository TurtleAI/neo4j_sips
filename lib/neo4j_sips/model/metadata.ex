defmodule Neo4j.Sips.Model.Metadata do
  ...
  defp expand_fields(fields) do
    fields
    |> Enum.map(fn {name, attributes} -> Neo4j.Sips.Model.Field.new(name, attributes) end)
    |> Enum.sort(&(&1.name < &2.name))
  end

  defp expand_relationships(relationships) do
    relationships
    |> Enum.map(fn {name, related_model} -> Neo4j.Sips.Model.Relationship.new(name, related_model) end)
    |> Enum.sort(&(&1.name < &2.name))
  end
end
